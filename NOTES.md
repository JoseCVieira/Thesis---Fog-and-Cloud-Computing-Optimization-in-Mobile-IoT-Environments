...
- Not used papers:
	...
	- API
		...
		- Supporting Internet-of-Things Analytics in a Fog Computing Platform (data placement to maximize number of satisfied users, does not consider migration)
...
- Check:
	...
	- Move with Me: Scalably Keeping Virtual Objects Close to Users on the Move (described above)
	...
	- Multitier Fog Computing With Large-Scale IoT Data Analytics for Smart Cities (described above, first efforts on mobile fog nodes)
	- A Game-Theoretic Approach to Coalition Formation in Fog Provider Federations(Profit-aware)
	- Multiobjective Optimization for Computation Offloading in Fog Computing (energy cost delay)
	- Asymptotically Optimal Algorithm for Online Reconfiguration of Edge-Clouds (cost)
	- Assessment of the Suitability of Fog Computing in the Context of Internet of Things (cost)
	- MOERA: Mobility-agnostic Online Resource Allocation for Edge Computing (cost)
	- On Reducing IoT Service Delay via Fog Offloading (delay)
	- Toward Hierarchical Mobile Edge Computing: An Auction-Based Profit Maximization Approach
	- A PSO Model with VM Migration and Transmission Power Control for Low Service Delay in the Multiple Cloudlets ECC Scenari (Transmission Delay and Processing Delay)
