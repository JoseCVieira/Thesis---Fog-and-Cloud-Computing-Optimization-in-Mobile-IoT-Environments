...
\begin{flalign*}
\begin{aligned}
L^T_q = \sum_{i=0}^{M-1}~ \sum_{j=i+1}^{M} A^L_{q,i}~ A^L_{q,j} & \sum_{l\in N}\sum_{k\in N} \left( fL_{l,k} \times R_{z,l,k} + \frac{m^B_{l^S_z,l^D_z} \times R_{z,l,k}}{f^B_{l,k} \times b - \sum_{r\in Z} m^B_{l^S_r,l^D_r} \times R_{r, l, k} } \right),\\[6pt]
& i<j,\\
